...
function profileRow(userId, d) {
    ...
    return {
        ...
        avatar_url: d.avatar_url ?? (d.images && d.images[0]) ?? null,
        ...
        blocked_profiles: [],
        member_ship_type: d.member_ship_type ?? "free",
        membership_expires_at: d.membership_expires_at ?? null,
        ...
        try {
            ...
            if (authError) {
                if (authError.message?.includes("already been registered") || authError.message?.includes("already exists")) {
                    ...
                    const user = existing?.users?.find((u) => u.email === d.email);
                    if (user) {
                        const { error: upError } = await supabase.from("profiles").upsert(profileRow(user.id, d), {
                            onConflict: "id",
                        });
                        if (upError)
                            console.log(`   ⚠️ Profile update error: ${upError.message}`);
                        else
