...
import { agefilter, intrestsfilter, setCategoryFilter } from '@/store/profileSlice'; 
...
export default function BottomFilterDialog({ visible, onClose }) {
  ...
  const reduxCategory = useSelector((state: any) => state?.profileSlice?.cetagoryfilter); // Ab ye string expect karega
  ...
  const categoryList = ["Casual dating",
	"Hookups",
	"Open to anything",
	"Friends first"];
  ...
  }, [visible, reduxInterests, reduxAge, reduxCategory]);
  ...
  const Reset = () => {
    ...
    dispatch(setCategoryFilter("")); // Empty string for category
      ...
      >
        ...
        <View style={styles.row}>
          {categoryList.map((cat) => (
            ...
            >
              <Text style={[styles.chipText, selectedCategory === cat && { color: Colors.white }]}>
                {cat}
              </Text>
            </TouchableOpacity>
          ))}
